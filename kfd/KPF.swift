...
open class KPF {
    ...
    public lazy var pmap_image4_trust_caches: UInt64? = {
        ...
        guard let tcString = cStrSect.addrOf("com.apple.load_trust_cache") else {
            return nil
        }
        
        guard let tcStringXREF = textExec.findNextXref(to: tcString, optimization: .noBranches) else {
            return nil
        }
        ...
        while true {
            if let adrp = AArch64Instr.Emulate.adrpLdr(adrp: textExec.instruction(at: pc)!, ldr: textExec.instruction(at: pc + 4)!, pc: pc) {
                if textExec.instruction(at: pc)!.bits(0...4) == 25 {
                    return adrp
                }
            }
            pc += 4
    ...
    public static var running: KPF? = {
        if let krnl = MachO.runningKernel {
            return KPF(kernel: krnl)
        }
    ...
    public required init?(kernel: MachO) {
        ...
        let pplText = kernel.pfSection(segment: "__PPLTEXT", section: "__text")
